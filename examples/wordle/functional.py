...
_SYSTEM_PROMPT = (
    ...
    "You are on turn 5, your final attempt.\n"
    "Think inside <think>...</think>.\n"
    "Then output exactly one guess inside <guess>[word]</guess>."
)


def _resolve_word_source_path(source: str) -> Path:
...
@lru_cache(maxsize=8)
def load_word_list(word_source: str = _DEFAULT_WORD_SOURCE_URL) -> list[str]:
    resolved_source = str(word_source).strip()
    if not resolved_source:
        raise ValueError("word_source cannot be empty.")

    if resolved_source.startswith(("http://", "https://", "file://")):
        with urlopen(resolved_source, timeout=30) as response:
            lines = response.read().decode("utf-8").splitlines()
        return _filter_five_letter_words(lines)

    with open(_resolve_word_source_path(resolved_source), encoding="utf-8") as handle:
...
def build_turn5_problem_rows(
    *,
    ...
    valid_words = load_word_list(word_source)
