...
def _matches_history(secret_candidate: str, history_rows: Sequence[tuple[str, str]]) -> bool:
    return all(
        compute_feedback(secret_candidate, guess) == feedback
        for guess, feedback in history_rows
    )


def _feedback_match_score(expected: str, actual: str) -> int:
    score = sum(
        1
        for expected_letter, actual_letter in zip(expected, actual)
...
def _constraint_fit_score(
    ...
    return sum(
        _feedback_match_score(feedback, compute_feedback(candidate, guess))
        for guess, feedback in history_rows
    )


def _secret_closeness_score(secret: str, guess: str) -> tuple[int, int]:
    feedback = compute_feedback(secret, guess)
    return (feedback.count("G"), feedback.count("Y"))


def _choose_turn5_history_guess(
    *,
    ...
    remaining_words = [
        ...
        if word not in used_guesses and word != secret
    ]
    if not remaining_words:
        raise ValueError("No remaining non-secret words left for turn-5 history.")

    if not history_rows:
        return rng.choice(remaining_words)

    consistent_words = [
        word for word in remaining_words if _matches_history(word, history_rows)
    ]
    candidate_pool = consistent_words or remaining_words

    best_score: tuple[int, int] | None = None
    best_words: list[str] = []
    for word in candidate_pool:
        score = (
            _constraint_fit_score(word, history_rows),
            *_secret_closeness_score(secret, word),
        )
...
def sample_turn5_history(
    *,
    ...
    while len(history_rows) < 4:
        guess = _choose_turn5_history_guess(
