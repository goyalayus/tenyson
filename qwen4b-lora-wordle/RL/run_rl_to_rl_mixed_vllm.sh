...
# 8x GPU launch: RL LoRA -> mixed turn 2-5 RL continuation (TRL GRPO + vLLM colocate).

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"

NOW="$(date +%Y%m%d_%H%M%S)"
RUN_ID="grpo_mixed_2_5_rl_init_vllm_${NOW}"

OUT_ROOT="${ROOT_DIR}/qwen4b-lora-wordle/outputs/RL"
mkdir -p "${OUT_ROOT}/${RUN_ID}"

BASE_MODEL="${BASE_MODEL:-Qwen/Qwen3-4B}"
INIT_ADAPTER_REPO="${INIT_ADAPTER_REPO:?set INIT_ADAPTER_REPO (existing RL adapter repo)}"
INIT_ADAPTER_REVISION="${INIT_ADAPTER_REVISION:-main}"
